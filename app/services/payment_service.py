...
def _payment_response(payment: Payment, order: Order) -> dict:
    return {
        "order_id": order.id,
...
def create_qr_payment(db: Session, order: Order) -> dict:
    ...
    if existing is not None:
        return _payment_response(existing, order)
    ...
    try:
        ...
        return _payment_response(existing, order)
    db.refresh(payment)
    return _payment_response(payment, order)
...
def process_webhook(db: Session, raw_body: bytes, signature: str, event_id: str) -> None:
    ...
    existing = db.query(Payment).filter(Payment.webhook_event_id == event_id).first()
    if existing is not None:
        return
